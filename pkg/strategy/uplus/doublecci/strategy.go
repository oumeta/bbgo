...
type Strategy struct {
	...
	*types.TradeStats  `persistence:"trade_stats"`
	...
	atr     *indicator.ATR
	cciFast *indicator.CCI
	cciSlow *indicator.CCI
	change  *indi.Slice
...
func (s *Strategy) AddKline(kline types.KLine) {
	...
	s.cciFast.PushK(kline)
	s.cciSlow.PushK(kline)
	s.atr.PushK(kline)
...
func (s *Strategy) klineHandler(ctx context.Context, kline types.KLine) {
	...
	longCondition := !changed && s.change.Last() == "-1" && s.cciFast.CrossOver(s.cciSlow).Last()

	shortCondition := !changed && s.change.Last() == "1" && s.cciFast.CrossUnder(s.cciSlow).Last()

	exitLongCondition := s.buyPrice > 0 && !longCondition && s.buyPrice*(1.-stoploss) >= lowf || s.buyPrice-atr*6 >= lowf || s.trailingCheck(lowf, "long")
