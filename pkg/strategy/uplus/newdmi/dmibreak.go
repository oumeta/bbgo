...
// DmiBreak -- when price breaks the previous pivot low, we set a trade entry
type DmiBreak struct {
	Symbol string
	Market types.Market
	...
	// limit sell price = DmiBreakPrice * (1 + BounceRatio)
	...
	// lastDmiBreak is the low that the price just break
	lastDmiBreak fixedpoint.Value
...
func (s *DmiBreak) Subscribe(session *bbgo.ExchangeSession) {
...
func (s *DmiBreak) initIndicators(store *bbgo.MarketDataStore) error {
...
func (s *DmiBreak) UpdateKline(kline types.KLine) {
...
func (s *DmiBreak) CalcAssetValue(price fixedpoint.Value) fixedpoint.Value {
	balances := s.session.GetAccount().Balances()
	return balances[s.Market.BaseCurrency].Total().Mul(price).Add(balances[s.Market.QuoteCurrency].Total())
}

func (s *DmiBreak) initTickerFunctions() {
	if s.IsBackTesting() {
...
func (s *DmiBreak) Bind(session *bbgo.ExchangeSession, orderExecutor *bbgo.GeneralOrderExecutor) {
...
// 处理逻辑
func (s *DmiBreak) Place(kline types.KLine) {
...
func (s *DmiBreak) pilotQuantityCalculation() {
	if s.lastLow.IsZero() {
		return
	}
