...
func (s *DmiMacd) Place(kline types.KLine) {
	...
	*/
	...
	short := down && (adx > 25) // && atrdiff && volumeBreak //&& !aboveEma //&& !aboveEma  && volumeBreak
	if short {
		sig = SELL
	}
	...
	//exitLongCondition := s.buyPrice > 0 && !longCondition || closeLong || kline.GetClose().Compare(s.currentTakeProfitPrice) > 0 || kline.GetClose().Compare(s.currentStopLossPrice) < 0 //|| stSignal == types.DirectionDown //kline.Close.Float64() > s.buyPrice+1.5*atr //|| s.trailingCheck(lowf, "long")
	exitLongCondition := s.buyPrice > 0 && (s.buyPrice-s.atr.Last()*2) >= lowf || closeLong || (s.buyPrice+s.atr.Last()*3) >= highf //|| stSignal == types.DirectionDown // || kline.Close.Float64() < s.sellPrice-1.5*atr //|| s.trailingCheck(highf, "short")

	exitShortCondition := s.sellPrice > 0 && (s.sellPrice-s.atr.Last()*3) >= lowf || closeShort || (s.sellPrice+s.atr.Last()*2) <= highf //|| stSignal == types.DirectionUp // || kline.Close.Float64() < s.sellPrice-1.5*atr //|| s.trailingCheck(highf, "short")
	fmt.Printf("time%s,close%.4f,dip:%.4f,dim:%.4f,adx:%.4f\n", kline.EndTime, kline.Close.Float64(), s.dmi.GetDIPlus().Last(), s.dmi.GetDIMinus().Last(), s.dmi.GetADX().Last())
	//fmt.Printf("ema:%.4f,hma:%.4f \n", s.emaSlow.Last(), s.hma.Last())
