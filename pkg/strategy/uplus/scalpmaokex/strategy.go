...
//		submitOrder := s.Position.NewMarketCloseOrder(percentage)
//		if submitOrder == nil {
//			return nil
//		}
//
...
//
//		_, err := s.SubmitOrders(ctx, *submitOrder)
//		return err
//	}
...
func (s *Strategy) Run(ctx context.Context, orderExecutor bbgo.OrderExecutor, session *bbgo.ExchangeSession) error {
	...
	store.OnKLine(func(kline types.KLine) {
		...
		//fmt.Println("s.buyPrice", s.buyPrice)
		//if s.buyPrice > 0 {
...
func (s *Strategy) klineHandler(ctx context.Context, kline types.KLine) {
	...
	fmt.Printf("sma:%4.f,bott2:%.4f,limit:%.4f,stop:%.4f,atr:%.4f", s.ma.Last(), bott2, limitLong, stopLong, atr)
