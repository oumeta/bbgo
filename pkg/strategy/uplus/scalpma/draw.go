...
func (s *Strategy) InitDrawCommands(store *bbgo.MarketDataStore, profit, cumProfit types.Series) {
...
func (s *Strategy) DrawIndicators(store *bbgo.MarketDataStore) *types.Canvas {
...
func (s *Strategy) Draw(store *bbgo.MarketDataStore, profit, cumProfit types.Series) {
	canvas := s.DrawIndicators(store)
